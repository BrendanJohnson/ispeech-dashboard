...
export default {
  ...
  render () {
    ...
    ctx.fillText(this.label, (newBox.x + (newBox.w / 2)), newBox.y-10);

    // Draw the new rectangle.
    ctx.rect(newBox.x, newBox.y-7, newBox.w, 2);
    ...
    ctx.textAlign = 'center';
    ctx.fillText(this.tagLabel, (newBox.x + (newBox.w / 2)), newBox.y + 6);
    //ctx.fillText(Math.floor(this.value), (newBox.x + (newBox.w / 2)), newBox.y - 13)

    // Draw the description
    ctx.fillStyle = '#000'
    ctx.font = '10px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(this.detailLabel, (newBox.x + (newBox.w / 2)), newBox.y+15);
